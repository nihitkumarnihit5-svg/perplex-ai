...
# ================= GOOGLE LOGIN FIX (CRITICAL) =================
# Ye line production mein HTTPS error aur Blocked login ko thik karti hai
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1, x_prefix=1)

app.secret_key = os.getenv("SECRET_KEY", "nihit_x_fire_ultra_pro_max_key")
...
def get_ai_response(msg, chat_id, user_email, base64_image=None):
    try:
        ...
        # 2. History for Context (Last 6 messages)
        ...
        Tone: Friendly, cool, and talks in Hinglish. Use 'bhai', 'theek hai', 'bro'.
        Memory: {user_context}
        Capabilities:
        - If the user asks for code, act as a Senior Full-Stack Developer.
        - If the user says 'yaad rakhna' or 'remember', acknowledge it.
        ...
        if any(x in msg.lower() for x in ["yaad rakhna", "remember", "mera naam"]):
            new_mem = f"{user_context} | {msg}"
            db.execute("INSERT OR REPLACE INTO user_memory VALUES(?,?)", (user_email, new_mem[-1000:]))
            db.commit()

        return reply
    except Exception as e: return f"Bhai, error aa gaya: {str(e)}"
...
def index():
    ...
    return '''<body style="background:#0e0e0e; color:white; display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh; font-family:sans-serif;">
              <h1 style="font-size:40px;">PERPLEX AI</h1>
...
@app.route("/login")
def login():
    # Force HTTPS scheme in redirect URI to fix 401 error
...
<head>
    ...
    <style>
        ...
        .logo { font-size: 20px; font-weight: 800; color: var(--accent); margin-bottom: 20px; }
        .new-chat { background: #2f2f2f; padding: 12px; border-radius: 12px; text-align: center; cursor: pointer; border: 1px solid #3d3d3d; }
        .chat-list { flex: 1; margin-top: 20px; overflow-y: auto; }
        .chat-item { padding: 12px; border-radius: 10px; cursor: pointer; font-size: 14px; margin-bottom: 5px; color: #b4b4b4; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .input-wrapper { background: #212121; border-radius: 24px; padding: 10px 15px; display: flex; border: 1px solid #333; }
    ...
    <div class="main">
        <div class="chat-container" id="chatBox">
            <div style="text-align:center; margin-top:15vh; color:#555;">
                <h2>Perplex AI Online</h2>
                <p>Coding and Memory Enabled.</p>
            </div>
        </div>
        <div class="input-area">
            <div class="input-wrapper">
                <input type="text" id="userInput" placeholder="Ask anything..." onkeypress="if(event.key=='Enter')send()">
                <button onclick="send()" style="background:none; border:none; color:var(--accent); cursor:pointer; font-size:20px;">➤</button>
            </div>
        </div>
    </div>
    <script>
        ...
        function send() {
            ...
            input.value = "";
            const fd = new FormData(); fd.append("chat", currentChatId); fd.append("msg", val);
        ...
        function clearAll() { if(confirm("Saare chats delete karun?")) fetch("/delete_history", {method:"POST"}).then(() => location.reload()); }
        loadChats();
    </script>
</body>
</html>
